...
describe("Parser.matchIf", () => {
  it("matches with a condition", () => {
    const p = pr.regex(/\d+/).matchIf(match => parseInt(match[0], 10) % 2 == 0).onFail("Expected an even number");
  ...
  it("can be called filter", () => {
    ...
    const p = pr.regex(/\d+/).filter((match, span) => span.endLine.xpos % 2 == 0).onFail("Expected an even length!");
